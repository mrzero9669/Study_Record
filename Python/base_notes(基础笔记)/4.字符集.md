...
&ensp;&ensp;&ensp;&ensp; 计算机只能处理数字，如果要处理字符串文本，就需要将文本转换为数字才能处理；那么就生成聊如下问题：
...
replace() 替换、bytearray.fromhex(string)、hex()、索引；
|参数/方法|使用方法|功能效果|
|-------|-------|-------|
